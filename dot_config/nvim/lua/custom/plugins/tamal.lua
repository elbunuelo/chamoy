...
local function open_tamal_popup()
  ...
  local title = "Tamal Task Manager"
  ...
  vim.keymap.set('n', '<Esc>', ':q<CR>', keymap_opts)
...
vim.api.nvim_create_user_command('Tamal', function()
  open_tamal_popup()
end, { desc = 'Open Tamal task manager popup' })

-- Add normal mode keybinding using <leader>t
vim.keymap.set('n', '<leader>t', ':Tamal<CR>', { desc = 'Open Tamal task manager', silent = true })
