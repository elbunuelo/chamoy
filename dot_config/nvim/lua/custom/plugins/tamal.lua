...
local function open_file_in_floating_window(file_path)
  ...
  if string.match(filename, '^%d%d%d%d%-%d%d%-%d%d_week%.md$') then
    -- Function to position cursor on the current day's line
    local function position_cursor_on_current_day(target_win, target_buf)
      ...
      -- Safety check for window and buffer validity
      if not (vim.api.nvim_win_is_valid(target_win) and vim.api.nvim_buf_is_valid(target_buf)) then
        return false
      end
      ...
      vim.api.nvim_win_set_option(target_win, 'cursorline', true)

      vim.notify('Positioned cursor at line ' .. target_line .. ' for ' .. current_day_name, vim.log.levels.INFO)
      return true
    end
    ...
    -- Try to position immediately (might not work if buffer is not fully loaded)
    position_cursor_on_current_day(win, buf)
    ...
    vim.api.nvim_create_autocmd({ 'BufEnter', 'BufWinEnter', 'BufReadPost' }, {
      ...
      -- Remove the 'once = true' to allow multiple attempts if needed
    })

    -- Also try with a series of delayed attempts to handle different timing scenarios
    local delays = { 100, 300, 600, 1000 }
